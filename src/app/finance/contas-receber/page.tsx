...
export default function FinanceContasReceberPage() {
  ...
  async function receber(id: string, cashboxId: string) {
    ...
    if (!res.ok) { alert(data.error ?? "Erro ao receber."); return; }
    load();
  }
  ...
  return (
    <div>
      <div className="mb-lg flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-h2 font-semibold text-[var(--text-primary)]">Contas a Receber</h1>
      ...
      <div className="mb-4 flex gap-2">
        <button type="button" onClick={() => setStatusFilter("pending")} className={`rounded-lg px-3 py-1.5 text-small font-medium ${statusFilter === "pending" ? "bg-primary-100 text-primary-700 dark:bg-primary-900/40 dark:text-primary-300" : "bg-neutral-100 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-300"}`}>Pendentes</button>
        <button type="button" onClick={() => setStatusFilter("received")} className={`rounded-lg px-3 py-1.5 text-small font-medium ${statusFilter === "received" ? "bg-primary-100 text-primary-700 dark:bg-primary-900/40 dark:text-primary-300" : "bg-neutral-100 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-300"}`}>Recebidas</button>
      </div>

      {loading && <Card padding="lg"><div className="flex items-center justify-center py-12"><div className="h-8 w-8 animate-spin rounded-full border-2 border-primary-600 border-t-transparent" /></div></Card>}
      {error && <Card padding="lg" className="border-red-200 dark:border-red-800"><p className="text-body text-red-600 dark:text-red-400">{error}</p><Button variant="primary" className="mt-4" onClick={load}>Tentar novamente</Button></Card>}
          ...
          {items.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-small">
                ...
                <tbody>
                  {items.map((r) => (
                    <tr key={r.id} className="border-b border-neutral-100 hover:bg-neutral-50 dark:border-neutral-800 dark:hover:bg-neutral-800/50">
                      <td className="p-lg text-[var(--text-secondary)]">{new Date(r.dueDate).toLocaleDateString("pt-BR")}</td>
                      <td className="p-lg font-medium text-[var(--text-primary)]">{r.description}</td>
                      <td className="p-lg text-[var(--text-secondary)]">{r.category ?? "—"}</td>
                      <td className="p-lg text-right font-mono"><MoneyValue value={r.amount} variant="entry" size="table" /></td>
...
function ReceberButton({ receivableId, onReceber, onSuccess }: Readonly<{ receivableId: string; onReceber: (id: string, cashboxId: string) => void; onSuccess: () => void }>) {
  const [cashboxes, setCashboxes] = useState<{ id: string; name: string }[]>([]);
  const [open, setOpen] = useState(false);
  const [cashboxId, setCashboxId] = useState("");
  const dialogRef = useRef<HTMLDialogElement>(null);

  useEffect(() => { fetch("/api/finance/cashboxes").then((r) => r.json()).then(setCashboxes).catch(() => setCashboxes([])); }, []);
  ...
  const handleReceber = () => {
    if (!cashboxId) return;
    onReceber(receivableId, cashboxId);
    close();
    setCashboxId("");
    onSuccess();
  };
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} className="inline-flex items-center gap-1 text-green-600 hover:underline"><CheckCircle className="h-4 w-4" /> Receber</button>
      <dialog
        ref={dialogRef}
        className="fixed inset-0 z-50 m-0 max-h-none max-w-none rounded-none border-0 bg-transparent p-0 [&::backdrop]:bg-black/50"
        aria-label="Receber conta"
        onCancel={close}
      >
        <button
          type="button"
        ...
        <div className="relative z-10 mx-auto mt-[10vh] w-full max-w-sm rounded-lg bg-[var(--background)] p-4 shadow-lg">
          <p className="mb-3 font-medium text-[var(--text-primary)]">Receber conta</p>
          <label htmlFor="receber-cashbox-select" className="mb-1 block text-small text-[var(--text-secondary)]">Caixa para crédito</label>
          <select
            id="receber-cashbox-select"
            value={cashboxId}
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button disabled={!cashboxId} onClick={handleReceber}>Confirmar recebimento</Button>
          </div>
        </div>
      </dialog>
    </>
